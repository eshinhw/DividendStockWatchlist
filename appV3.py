...
def add():
    ...
    try:
        (cp, historical_dy) = _get_close_and_historical_div_yield(s)
        last_year = dt.datetime.today().year - 1
        div_yield = round(100 * DIV_DATA[s.upper()][0][str(last_year)] / cp, 2)
    except:
        _clear_input()
    ...
    c.execute(
        f"INSERT INTO {TABLE_NAME} VALUES (:symbol, :target_price, :current_price, :div_yield, :historical_avg_div_yield)",
        {
            ...
            'current_price': cp,
            'div_yield': div_yield,
            'historical_avg_div_yield': historical_dy
        })
...
def query():
    ...
    watchlist_tree['columns'] = ("ID", "SYMBOL", "ALERT PRICE",
                                 "MARKET PRICE", "CURRENT DIV Y", "10Y AVG DIV Y")
    ...
    watchlist_tree.heading("MARKET PRICE",
                           text="Market Price",
                           anchor=tk.CENTER)
    ...
    buylist_tree.heading("MARKET PRICE",
                         text="Market Price",
                         anchor=tk.CENTER)
